...
class App extends React.Component {
  ...
  setActiveModal(activeModal = null, data = null) {
    let modalHistory = this.state.modalHistory
      ...
      : [];
    if (data !== null)
      this.setState({ data: { shortened: data.shortened, link: data.link } });
    else
      setTimeout(() => {
        this.setState({
  ...
  AndroidBackButton = () => {
    if (this.state.activePanel !== ROUTES.HOME && this.state.online === true) {
        ...
        <ModalCard
          ...
          actions={
            <Div style={{ display: "flex", width: "100%", marginTop: "-20px" }}>
              ...
              >
                <Button size="l" stretched before={<Icon28LinkOutline />} />
              </a>
              <Button
                size="l"
                stretched
                mode="secondary"
                disabled={this.state.disabledCopy ? true : false}
                style={{ width: "50%" }}
                onClick={() => {
