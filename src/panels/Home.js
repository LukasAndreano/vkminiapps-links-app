...
class Home extends React.Component {
          ...
          data.result.links.map((el) => {
            links.push(
              <ContentCard
                ...
                subtitle={
                  messages[
                    Math.floor(Math.random() * (messages.length - 0)) + 0
                  ]
                }
                header={el.shortened}
                key={el.id}
                ...
                onClick={() => {
                  this.props.setActiveModal("link", {
                    ...
                    link: el.link.replace(/~TAG~/gi, "#").replace(/[%]/gi, "&"),
                  });
                }}
                caption={
                  "Оригинал: " +
                  el.link.replace(/~TAG~/gi, "#").replace(/[%]/gi, "&")
                }
              />
            );
          });
  ...
  componentWillUnmount() {
    this.setState({ formstatus: { bottom: "", status: "default", url: "" } });
  }
  ...
  addToFavorites() {
    if (this.state.isFavorite == 0) {
      ...
      this.props.setSnackbar("Сервис уже в избранном! Спасибо ❤️", 2000, true);
    }
  }
      ...
      if (urlregex.test(textval.trim())) {
        ...
        if (
          this.state.blacklisted.includes(
            new URL(
              "https://" + textval.replace(/(^\w+|^)/, "").replace(/:\/\//, "")
            ).hostname.replace("www.", "")
          )
        )
          this.setState({
            formstatus: {
              bottom: "Хочешь сократить уже сокращённую ссылку? Забавно!",
              status: "error",
              url: "",
            },
            url: textval.trim(),
        ...
        {this.state.spinner === false && (
          <Epic
            ...
            tabbar={
              <Tabbar>
                ...
                <TabbarItem
                  selected={this.state.activeStory === "raiting"}
                  data-story="raiting"
                  onClick={this.openPage}
                  text="Рейтинг"
                >
          ...
          >
            <Panel id="links" activeStory="links">
              <PanelHeader separator={false}>Сокращатель</PanelHeader>
              {this.props.user !== null && (
                <Group separator="hide">
                  <RichCell
                    before={
                      <Avatar size={48} src={this.props.user.photo_200} />
                    }
                    caption={"Сокращено ссылочек: " + this.state.points}
                    disabled
                  >
                ...
                <FormLayout
                  onSubmit={(e) => {
                    ...
                    if (
                      ...
                      this.state.formstatus.status === "default" &&
                      this.state.url !== this.state.shortened &&
                      this.state.service !== ""
                    ) {
                      this.setState({
                        disabled: true,
                      ...
                      fetch2(
                        "getShortLink",
                        "serivce=" +
                          encodeURI(this.state.service) +
                          "&url=" +
                          encodeURI(
                            this.state.formstatus.protocol +
                              "://" +
                        ...
                        .then((data) => {
                          if (
                            ...
                            if (data.result.already_shortened == false) {
                              let url = this.state.formstatus.url;
                              let arr = [];
                                ...
                                arr.push(
                                  <ContentCard
                                    subtitle={
                                      messages[
                                        Math.floor(
                                          Math.random() * (messages.length - 0)
                                        ) + 0
                                      ]
                                    }
                                    header={data.result.url}
                                    onClick={() => {
                                      this.props.setActiveModal("link", {
                                        shortened: data.result.url,
                                        link: url,
                                      });
                                    }}
                                    caption={"Оригинал: " + url}
                                  />
                                );
                              } else {
                                if (this.state.links.length >= 50) {
                            ...
                            this.setState({ shortened: data.result.url });
                            ...
                            this.setState({
                              formstatus: {
                                bottom: "Не-а. Введи что-нибудь другое!",
                                status: "error",
                              },
                              url: this.state.formstatus.url,
                              formDisabled: false,
                            });
                ...
                >
                  ...
                  >
                    <Input
                      type="text"
                      maxLength="400"
                      disabled={this.state.formDisabled ? true : false}
            ...
            <Panel id="history" activeStory="history">
              <PanelHeader separator={false} style={{ marginBottom: 5 }}>
                История ссылок
              </PanelHeader>
            ...
            <Panel id="raiting" activeStory="raiting">
              ...
              >
                {this.props.user !== null && (
                  <Group
                    separator="hide"
                    header={<Header mode="secondary">Твоя статистика</Header>}
                  >
                    <RichCell
                      before={
                        <Avatar size={48} src={this.props.user.photo_200} />
            ...
            <Panel id="about" activeStory="about">
              {platform() !== "ios" && (
                <PanelHeader separator={false}>О сервисе</PanelHeader>
              )}
              {platform() === "ios" && (
                <div>
                  <PanelHeader
                    ...
                    left={
                      ...
                      >
                        {this.state.isFavorite == 0 && <Icon28LikeOutline />}{" "}
              ...
              <Group>
                ...
                <Div style={{ marginBottom: -10 }}>
                  <SimpleCell
                    onClick={() => {
                      bridge.send("VKWebAppOpenApp", { app_id: 7722891 });
                    }}
                    after={<Icon28ChevronRightOutline />}
                    before={<Avatar mode="image" src={img1} />}
                  ...
                  <SimpleCell
                    onClick={() => {
                      bridge.send("VKWebAppOpenApp", { app_id: 7784361 });
                    }}
                    after={<Icon28ChevronRightOutline />}
                    before={<Avatar mode="image" src={img2} />}
